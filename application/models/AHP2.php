...
class AHP2 extends CI_Model {
	...
	public function	perbandingan_alternatif($data, $nilai){
		...
		foreach ($this->db->get('kriteria')->result() as $nilai_kriteria => $kriteria) {
			foreach ($nilai as $key_nilai => $value_nilai) {
				if($kriteria->id_kriteria == $value_nilai->kriteria){
						//---------------------------------------//
						foreach ($data as $key => $value) {
							foreach ($nilai as $keys => $values) {
								
								if($value->nip == $values->alternatif2 and $kriteria->id_kriteria == $values->kriteria){

									$arr_bobot[$kriteria->id_kriteria][$key][] = $values->bobot;
								}//end if
							}
						}
						//---------------------------------------//						
					...
					/*	$this->pre($arr_bobot);
							...
							foreach ($data as $key => $value) {
								foreach ($data as $keys => $values) {
										...
										if($key  == $keys){
											...
											$val = $arr_bobot[$kriteria->id_kriteria][$key][$keys];
										}else{
											$val = 1 / $arr_bobot[$kriteria->id_kriteria][$keys][$key];
											#$val = '1 /'.$arr_bobot[$kriteria->id_kriteria][$keys][$key];
										}

										$arr_result[$kriteria->id_kriteria][$keys][$key] = round($val,4);
										#$arr_result[$kriteria->id_kriteria][$keys][$key] = $val;
									/*}else{
						...
						//---------------------------------------//
						//---------------------------------------//
						foreach ($arr_result[$kriteria->id_kriteria] as $key => $value) {
							$arr_total[$kriteria->id_kriteria][$key] = 0;
							foreach ($arr_result[$kriteria->id_kriteria][$key] as $keys => $values) {
								
								$arr_total[$kriteria->id_kriteria][$key] += round($arr_result[$kriteria->id_kriteria][$keys][$key],4);
	...
	public function normalisasi_prioritas($data, $nilai, $hasil){
		...
		foreach ($this->db->get('kriteria')->result() as $key => $kriteria) {
			foreach ($nilai as $key_nilai => $value_nilai) {
				if($value_nilai->kriteria == $kriteria->id_kriteria){
			
			$normalisasi[$kriteria->id_kriteria] = array();
			$arr_jumlah[$kriteria->id_kriteria] = array();
			$arr_total[$kriteria->id_kriteria] = array();
			$arr_prio[$kriteria->id_kriteria] = array();

			$jumlah_arr[$kriteria->id_kriteria] = count($hasil['hasil'][$kriteria->id_kriteria]);
			
			foreach ($hasil['hasil'][$kriteria->id_kriteria] as $key => $value) {

				$jumlah[$kriteria->id_kriteria] = 0;
				$priorotas[$kriteria->id_kriteria] = 0;

				
				

				foreach ($hasil['hasil'][$kriteria->id_kriteria][$key] as $keys => $values) {

					$normalisasi[$kriteria->id_kriteria][$key][$keys] = round($hasil['hasil'][$kriteria->id_kriteria][$key][$keys] / $hasil['total_bawah'][$kriteria->id_kriteria][$keys],4);
					$jumlah[$kriteria->id_kriteria] += round($normalisasi[$kriteria->id_kriteria][$key][$keys],4);

				}

				$priorotas[$kriteria->id_kriteria] = $jumlah[$kriteria->id_kriteria] / $jumlah_arr[$kriteria->id_kriteria];

				$arr_prio[$kriteria->id_kriteria][] = round($priorotas[$kriteria->id_kriteria],4);
				$arr_jumlah[$kriteria->id_kriteria][] = $jumlah[$kriteria->id_kriteria];
			}
			...
			foreach ($normalisasi[$kriteria->id_kriteria] as $key => $value) {

				$total[$kriteria->id_kriteria] = 0;

				foreach ($normalisasi[$kriteria->id_kriteria][$key] as $keys => $values) {
					
					$total[$kriteria->id_kriteria] = $normalisasi[$kriteria->id_kriteria][$keys][$key] + $total[$kriteria->id_kriteria];

				}
				$arr_total[$kriteria->id_kriteria][] = round($total[$kriteria->id_kriteria],4);
			}
	...
	public function	perbandingan_kriteria($data, $kriteria){
		...
		foreach ($kriteria as $key => $value) {
			foreach ($data as $keys => $values) {
				...
				if($value->id_kriteria == $values->kriteria2){
...
/*		$total_jumlah = 0;
		...
		$jumlah_cm = round(($jumlah_cm / $jumlah) / $jumlah,4);
		...
		return array('hasil'=> $arr_result, 
			...
			//ranking($data['alternatif'], $data['nilai'], $data['prioritas_kriteria'])
	function ranking($alternatif, $prio_kriteria, $prio_alternatif){
		$arr_result = array();
		foreach ($alternatif as $key => $value) { //guru
			
			foreach ($prio_kriteria as $keys => $values) { //prioritas_alternatif
